...
function colorScale(min_color, max_color, min_num, max_num, num) {
...
function setColorScaleOnElements(elements, min_color, max_color, zero_color, null_color) {
  ...
  elements.forEach(td => {
    ...
    if (td.innerText === "" || Number.isNaN(num) || num < 0) {
      td.innerText = ""; // 無効な文字列なら、セル内を空にする。
      changeColor(td, null_color);
    } else if (num === 0) {
      changeColor(td, zero_color);
    } else {
      changeColor(td, colorScale(min_color, max_color, min_num, max_num, num))
    }
  });
}

/* 表に対し、ボディを行方向にカラースケール化する。 */
function setColorScaleHorizontal(target_class, min_color, max_color, zero_color, null_color) {
  /* 行ごとに処理。 */
  document.querySelectorAll("tr").forEach(tr => {
    ...
    /* 各セルの着色。 */
    if (td_nodes) {
      setColorScaleOnElements(td_nodes, min_color, max_color, zero_color, null_color);
    }
  });
}

/* 表に対し、ボディを列方向にカラースケール化する。 */
function setColorScaleVertical(target_class, min_color, max_color, zero_color, null_color) {
  /* 列数を取得。 */
...
function setColorScale({ is_horizontal,
                         ...
                         null_color = "#ffffff" }) {
  if (is_horizontal) {
    setColorScaleHorizontal(target_class, min_color, max_color, zero_color, null_color);
  } else {
    setColorScaleVertical(target_class, min_color, max_color, zero_color, null_color);
  }
}
